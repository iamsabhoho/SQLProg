...
SELECT album_name
FROM album
WHERE album_name LIKE "l%" OR (album_name LIKE "s%" AND album.album_name LIKE "%g");

#Exercises
SELECT album_name
FROM album
WHERE album_name LIKE "l%" OR album_name LIKE "s%" OR album.album_name LIKE "p%"; #1
SELECT album_name
FROM album
WHERE (album_name LIKE "l%" OR album_name LIKE "s%") AND album.album_name LIKE "%g"; #2
SELECT *
FROM album
WHERE album_id != 1 AND album.album_id != 3; #3
SELECT *
FROM album
WHERE album_id > 2 AND (album.album_id != 4 AND album.album_id != 6); #4
SELECT *
FROM album
WHERE album_name NOT LIKE "L%"; #5
SELECT *
FROM album
WHERE album_name LIKE "s%" AND album.album_name NOT LIKE "%)"; #6
SELECT artist_name
FROM artist
WHERE artist_name LIKE "%es" XOR artist.artist_name LIKE "the%"; #7
SELECT artist_id
FROM album
WHERE album_name = "Brotherhood"; #8 sth here
...
SELECT
  ...
  artist_id
FROM track
ORDER BY time DESC, track_name DESC
LIMIT 5; #2
...
# SET @OLD_FOREIGN_KEY_CHECKS = @@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS = 0;
# DELETE FROM track, album, artist
# USING artist, album, track
# WHERE
#   artist_name = "The rolling stones" AND
...
INSERT INTO track (track_id, track_name, artist_id, album_id, time) VALUES (0, 'Release the Pressure', 7, 8, '7.39');
INSERT INTO track VALUES (1, 'Afro-Melt', 7, 8, '7.33');
INSERT INTO track VALUES (2, 'Melt', 7, 8, '5.21');
INSERT INTO track VALUES (3, 'Song of Life', 7, 8, '6.55');
INSERT INTO track VALUES (4, 'Original', 7, 8, '6.00');
INSERT INTO track VALUES (5, 'Black Flute', 7, 8, '3.46');
INSERT INTO track VALUES (6, 'Space Shanty', 7, 8, '7.15');
INSERT INTO track VALUES (7, 'Inspection Check one', 7, 8, '6.30');
INSERT INTO track VALUES (8, 'Storm 3000', 7, 8, '5.44');
INSERT INTO track VALUES (8, 'Bonus Track', 7, 8, '1.22');
...
FROM album
  INNER JOIN track USING (artist_id, album_id)
WHERE album.artist_id = 7 AND album.album_id = 8;

#4
SELECT sum(time)
FROM album
  INNER JOIN track USING (artist_id, album_id)
WHERE album.artist_id = 4;

#5
SELECT * FROM track WHERE track_name = 'Original';
UPDATE track SET time = 6.22
WHERE artist_id = 7 AND album_id = 8;
...
SELECT artist_name, track_name FROM track INNER JOIN artist USING (artist_id) WHERE time = 11;